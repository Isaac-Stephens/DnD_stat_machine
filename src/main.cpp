  ...
  while (!glfwWindowShouldClose(window)) {
        ...
        if (ImGui::BeginTabItem(myCharacter.getName().c_str())) {
          ...
          if (ImGui::BeginTable("AbilityScoresTable", 7,
            ...
            for (int i = 0; i < 6; ++i) {
              ...
              switch (i) {
              case 0:
                mod = myCharacter.getAbilityMod(myCharacter.getStr());
                break;
              case 1:
                mod = myCharacter.getAbilityMod(myCharacter.getDex());
                break;
              case 2:
                mod = myCharacter.getAbilityMod(myCharacter.getCon());
                break;
              case 3:
                mod = myCharacter.getAbilityMod(myCharacter.getInt());
                break;
              case 4:
                mod = myCharacter.getAbilityMod(myCharacter.getWis());
                break;
              case 5:
                mod = myCharacter.getAbilityMod(myCharacter.getChr());
                break;
              }
              ImGui::TableSetColumnIndex(i + 1);
              ImGui::Text("%+d", mod);
            ...
            if (ImGui::BeginTable("SavingThrows", 3,
                                  ImGuiTableFlags_Borders |
                                      ImGuiTableFlags_RowBg)) {
              ImGui::TableSetupColumn("Saving Throws:",
                                      ImGuiTableColumnFlags_WidthStretch);
              ImGui::TableSetupColumn("Proficient?",
              ...
              for (auto &[save, isProficient] : saves) {
                ...
                int bonus =
                    myCharacter.getAbilityMod(myCharacter.getAbility(save));
                if (isProficient)
            ...
            if (ImGui::BeginTable("Proficiencies", 3,
              ...
              for (auto &[skill, isProficient] : proficiencies) {
                ...
                if (it != table.end()) {
                  string abilityName =
                      it->second; // e.g., "dexterity", "wisdom"
    ...
    if (show_dice_box) {
      static std::array<int, 8> dice = {
          0, 0, 0, 0, 0,
          0, 0, 0}; // Number of dice to roll {d100,d20,d12,d10,d8,d6,d4,custom}
      ...
      vector<std::pair<int, const char *>> diceTypes = {
          {100, "D-100s:"}, {20, "D-20s:"}, {12, "D-12s:"}, {10, "D-10s:"},
          {8, "D-8s:"},     {6, "D-6s:"},   {4, "D-4s:"}};
    ...
    if (show_settings) {
      ...
      ImGui::Text("Application average %.3f ms/frame (%.1f FPS)",
                  1000.0f / io.Framerate, io.Framerate); // Display framerate
      ImGui::End();
    }
